...
function GSTRFiling() {
    ...
    const client_id = client_Information.clientId;
    ...
    const myGSTN = {
        GSTIN: client_Information.gstin
        , State: fetchStateCode(client_Information.gstin === null ? `` : client_Information.gstin.slice(0, 2))
    };
    ...
    async function nextInvoiceID() {
        ...
        try {
            const response = await fetch(`${url_}/highest-invoice-id?bill_from_GSTIN=${myGSTN.GSTIN}&invoicestartdate=${FinancStartyear}-04-01 &invoiceenddate=${FinancStartyear + 1}-03-31`, requestOptions);
            if (response.status === 200) {
    ...
    async function saveInvoice(e, invoiceid, invID) {
        ...
        const saveObject = {
            ...
            bill_from_GSTIN: client_Information.gstin,
        ...
        switch (inputname) {
            ...
            case 'supply':
                switch (value) {
                    ...
                    case 'deemedexport':
                        updatedArray[index] = {
                            ...updatedArray[index], subtype: "Export", deemexport: true, sez: false,
                    ...
                    case 'sez':
                        updatedArray[index] = {
                            ...updatedArray[index], subtype: "SEZ", deemexport: false, sez: true,
                            rate: 0, igst_amount: 0, cgst_amount: 0, sgst_amount: 0,
                            total_inv_amount: updatedArray[index].total_taxiable_value !== 0 && updatedArray[index].total_taxiable_value
                        };
                        break;
    ...
    function createInvoice(e, invoiceid, option, id) {
        ...
        switch (option) {
            ...
            case "Edit":
                if (id) {
                    if (filednotfiled) {
                        ...
                        Navigate("invoice", { state: { invoiceid: invoiceid, month: month, year: year, option: option, id: id, client_Information: client_Information } })
                    }
                }
                ...
                if (id) {
                    if (filednotfiled) {
                        if (name === "originalist") {
                            Navigate("invoice", {
                                state: {
                                    invoiceid: invoiceid, month: month, year: year, option: option,
                                    id: id, amendid: invoices[index].amended && amendid[0].id, client_Information: client_Information
                                }
                            })
                        }
                            ...
                            Navigate("invoice", {
                                state: {
                                    invoiceid: invoiceid, month: month, year: year, option: option,
                                    id: origid[0].id, amendid: id, client_Information: client_Information
                                }
                            })
                        }
                    }
            ...
            case "View":
                ...
                break;
            default: Navigate("invoice", { state: { invoiceid: nextinvid, month: month, year: year, option: option, client_Information: client_Information } })
        }
        ...
        <div className='mt-4 '>
            <div className={`mb-3 ${style.maindiv}`}>
                <div className={`${style.headerleft}`}>
                    <div style={{ cursor: "pointer" }}> <h1><b onClick={(e) => {
                        e.preventDefault();
                        Navigate(-1);
                    }}  >
                        &#8617;&nbsp;
