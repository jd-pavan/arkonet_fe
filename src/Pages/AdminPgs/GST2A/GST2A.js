...
function GST2A() {
  ...
  const myGSTN = {
    ...
    , State: fetchStateCode(client_Information.gstin.slice(0, 2))
  };
  ...
  async function getFiledStatus() {
    ...
    try {
      const response = await fetch(`${url_}/GST_Statusfilednotfiled/${user_id}/${client_Information.clientId}/${year}/GSTR-3B`, requestOptions);
  ...
  async function handleToggle() {
    ...
    if (result.isConfirmed) {
      ...
      const response = await fetch(`${url_}/GSTupdateFiledNotFiled/${user_id}/${client_Information.clientId}/${month} ${year}/${year}/GSTR-3B`, requestOptions)
      if (response.status === 200) {
        Swal.fire("Done.", "GSTR-2A Filled.", "success")
        window.location.reload();
      } else {
        Swal.fire("Failed.", "GSTR-2A not Filled.", "error")
      }
      // window.location.reload();
  ...
  return (
    <div className='m-4 w-100'>
      ...
      <div className=''>
        ...
        <div className={`${style.tablediv} mr-4`}>
                  ...
                  invoices.map((invoice, index) => {
                    return (
                      <tr className={`${style.valuerow} ${invoice.cancelled && `${style.cancelled}`} ${invoice.invoiceImport && `${style.disabledrow}`} ${(invoice.deemexport || invoice.sez) && `${style.exported}`} ${invoice.amended && `${style.amended}`}`}>
                        ...
                        <td className={`${style.name}`}><input name={`bill_to_Name__${invoice.invoiceid}`} value={invoice.bill_from_Name} /></td>
