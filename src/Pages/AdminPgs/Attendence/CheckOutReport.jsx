...
const CheckOutReport = () => {
  ...
  const GetCheckOutList = async () => {
    try {
      ...
      const response = await fetch(`${url_}/working-time-SubUser-CheckOutList-DateBetween?startDate=${CheckOutDate}&endDate=${CheckOutDate}&category=Sub User&userid=${user_id}`, requestOptions)
          ...
          <tbody>
            {checkOutData.map((item, index) => (
              <tr>
                ...
                <td>{getCurrentDateTime(item.startTime, item.endTime)}</td>
              </tr>
            ))}
