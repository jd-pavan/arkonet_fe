...
import { GetAPIFunction } from './../../../APIsFunctions/GetAPIFunction'
...
function CreateInvoice() {
    ...
    const [custumersAddress, setCustumersAddress] = useState([]);
    ...
    const myGSTN = { bill_from_GSTIN: client_Information.gstin, bill_from_State: fetchStateCode(client_Information.gstin.slice(0, 2)) };
    ...
    const [invoice_fields, setinvoice_fields] = useState({
        ...
        bill_from_Name: client_Information.gstinname === null ? "" : client_Information.gstinname,

        bill_from_GSTIN: client_Information.gstin === null ? "" : client_Information.gstin,

        bill_from_State: fetchStateCode(client_Information.gstin.slice(0, 2)),
    ...
    // console.log(fetchStateCode(client_Information.gstin.slice(0, 2)))
    const InvoiceDATAToPrint = {
        ...invoice_fields,

        Client_GSTIN_Email: client_Information.email,
        Client_GSTIN_PhoneNo: client_Information.mobile,
        QR_Code: clientBankDetails.qrCode,
        UPI_ID: clientBankDetails.upiId,
        Bank_Name: clientBankDetails.bank_name,
        Bank_Branch: clientBankDetails.accountName,
        Account_No: clientBankDetails.accountNumber,
        IFSC_Code: clientBankDetails.ifsc,
        total_dis_amount: 0,
        pre_tax_amount: 0,
        cess_amount: 0,
        post_amount: 0,
    ...
    async function getStateFromPin(pinCode) {
        ...
        try {
            const response = await fetch(`https://api.postalpincode.in/pincode/${pinCode}`, requestOptions)
    ...
    function handleChange(e) {
        ...
        switch (name) {
            ...
            case 'ship_to_Pincode':
                ...
                break;
            case 'bill_to_GSTIN':
                const CustomerGSTIN = value.toUpperCase();
                setinvoice_fields({ ...invoice_fields, [name]: CustomerGSTIN })
                if (value.length === 15) {
                    GetCustomerAddressByGSTIN(CustomerGSTIN);

                }


                break;
            case 'cess_non_advol_amount':
                const cess_non_advol_amount = value.replace(/\D/g, "");
                setinvoice_fields({ ...invoice_fields, [name]: cess_non_advol_amount })



                break;
            case 'cess_advol_amount':
                const cess_advol_amount = value.replace(/\D/g, "");
                setinvoice_fields({ ...invoice_fields, [name]: cess_advol_amount })
    ...
    const ClickedButton = async (btnClicked) => {

        if (invoice_fields.documentdate === "") {
            Swal.fire("", "Select document date.!", "warning")
        } else {
            setClickedBtn(btnClicked);
            saveInvoice();
            SaveAddress(
                invoice_fields.bill_from_Name,
                invoice_fields.bill_from_GSTIN,
                invoice_fields.bill_from_State,
                invoice_fields.dispatch_from_Address,
                invoice_fields.dispatch_from_Place,
                invoice_fields.dispatch_from_Pincode
            );
            SaveAddress(
                invoice_fields.bill_to_Name,
                invoice_fields.bill_to_GSTIN,
                invoice_fields.bill_to_State,
                invoice_fields.ship_to_Address,
                invoice_fields.ship_to_Place,
                invoice_fields.ship_to_Pincode
            );
            ...
            //     }, 4000);
            // } else {
            //     // await Swal.fire("Record save Successfully")
            //     Navigate(-1);
            // }
            ...
            if (btnClicked !== "Submit") {
                setTimeout(() => {
                    console.log("Printing...");
                    window.print();
                    // After printing, handle navigation
                    setTimeout(() => {
                        Navigate(-1); // Assuming Navigate function handles navigation
                    }, 1000); // Adjust delay as needed
                }, 4000);
                setButtonStatus(true);
            } else {
                // If "Submit" button clicked, directly navigate
                Navigate(-1);
                setButtonStatus(true);
            }
        }
    ...
    const SaveAddress = async (
        ...
        Address_GSTIN_No,
        Address_GSTIN_State,
        Address_GSTIN_Address,
        Address_GSTIN_Place,
        Address_GSTIN_Pincode
    ) => {

        // console.log(Address_GSTIN_Name,
        //     Address_GSTIN_No,
        //     Address_GSTIN_State,
        //     Address_GSTIN_Address,
        //     Address_GSTIN_Place,
        ...
        try {
            ...
            const raw = JSON.stringify({
                "bill_Name": Address_GSTIN_Name,
                "bill_GSTIN": Address_GSTIN_No,
                "bill_State": Address_GSTIN_State,
                "ship_Address": Address_GSTIN_Address,
                "ship_Place": Address_GSTIN_Place,
                "ship_Pincode": Address_GSTIN_Pincode
            });
            ...
            const response = await fetch(`${url_}/save/gstin/address`, requestOptions)
            const result = await response.json();
            console.log(result)
        } catch (error) {
            console.log(error)
    ...
    async function saveInvoice(btnClicked) {

        switch (option) {
            case 'Create':


                createInvoice(btnClicked)
                break;
            case 'Amend':
    ...
    useEffect(() => {
        ...
        FetchIDS()
        GetClientBankData();
        GetClientsAddressByGSTIN();
    }, [])

    const GetCustomerAddressByGSTIN = async (custumerGSTIN) => {
        try {
            const myHeaders = new Headers();
            myHeaders.append("Authorization", `Bearer ${storedToken}`);
            ...
            const response = await fetch(`${url_}/get/${custumerGSTIN}`, requestOptions)
            const result = await response.json();
            console.log("CustomerAddressByGSTIN :", result)
            if (result.addressList.length < 1) {
                if (result.clientData === null) {
                    setCustumersAddress([]);
                    setinvoice_fields(prevState => ({
                        ...prevState,
                        bill_to_Name: "",
                        bill_to_State: "",
                    }));
                } else {
                    setCustumersAddress([{ ship_Address: result.clientData.gstinaddress }]);
                    setinvoice_fields(prevState => ({
                        ...prevState,
                        bill_to_Name: result.clientData.gstinname,
                        bill_to_State: fetchStateCode(custumerGSTIN.slice(0, 2)) ? "" : fetchStateCode(custumerGSTIN.slice(0, 2)),
                ...
                console.log(result.addressList[0].bill_State)
                setCustumersAddress(result.addressList);
                setinvoice_fields(prevState => ({
                    ...prevState,
                    bill_to_Name: result.addressList[0].bill_Name,
                    bill_to_State: result.addressList[0].bill_State
                }));

            }
            // console.log(fetchStateCode(custumerGSTIN.slice(0, 2)));
        } catch (error) {
            console.log(error)
        }
    }
    const GetClientsAddressByGSTIN = async () => {
        try {
            const myHeaders = new Headers();
            myHeaders.append("Authorization", `Bearer ${storedToken}`);
            ...
            const response = await fetch(`${url_}/get/${client_Information.gstin}`, requestOptions)
            const result = await response.json();
            console.log("GetClientsAddressByGSTIN:", result)
            if (result.addressList.length < 1) {
                setClientsAddress([{ ship_Address: result.clientData.gstinaddress }]);
            } else {
                setClientsAddress(result.addressList);
            }
    ...
    const SETNameClientPlaceOnAddress = async (data) => {
        // setinvoice_fields(prevState => ({
        //     ...prevState,
        //     dispatch_from_Address: data.ship_Address,
        //     dispatch_from_Place: data.ship_Place,
        //     dispatch_from_Pincode: data.ship_Pincode
        // }));
        const state = await getStateFromPin(data.ship_Pincode);
        console.log(state)
        console.log(data)
        if (state) {
            ...
            // setinvoice_fields({ ...invoice_fields, dispatch_from_State: state })
            // name === "ship_to_Pincode" && setinvoice_fields({ ...invoice_fields, ship_to_State: state })
        }
        else {
            Swal.fire({
            ...
            setinvoice_fields({ ...invoice_fields, dispatch_from_State: "" })
            // name === "ship_to_Pincode" && setinvoice_fields({ ...invoice_fields, ship_to_State: "" })
        }
    }
    const SETNameCustomerPlaceOnAddress = async (data) => {
        console.log(data)
        // setinvoice_fields(prevState => ({
        //     ...prevState,

        //     bill_to_State: data.bill_State,
        //     ship_to_Address: data.ship_Address,
        //     ship_to_Place: data.ship_Place,
        //     ship_to_Pincode: data.ship_Pincode
        // }));
        ...
        if (state) {
            ...
            // setinvoice_fields({ ...invoice_fields, dispatch_from_State: "" })
            setinvoice_fields({ ...invoice_fields, ship_to_State: "" })
        }
    }
    ...
    const divideData = (data, chunkSize) => {
        const dividedData = [];
        for (let i = 0; i < data.length; i += chunkSize) {
            dividedData.push(data.slice(i, i + chunkSize));
        }
        return dividedData;
    };

    const dividedData = divideData(childData, 9);
    ...
    const GetClientBankData = async () => {
        try {
            ...
            const response = await fetch(`${url_}/client-bank-details/pan/${client_Information.pan}`, requestOptions)
            const result = await response.json();
            // console.log(result)
            if (response.status !== 200) {
                Swal.fire("", "Client has not updated bank details!!", "warning")
    ...
    const PrintInvoice = async () => {
        const result = await GetClientBankData();
        console.log(result)
        // if (result) {
        ...
        <>
            <div className={`${style.maincontainer}`}>
                        ...
                        <div className={`${style.row}`}>
                            ...
                            <div className='col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 d-flex'>
                                <span className="w-50">
                                    ...
                                    <input name="documentdate" value={invoice_fields.documentdate} onChange={handleChange} type="date" max={todatyDate()} />
                                </span>
                                <span className="w-50">
                                ...
                                <tbody>
                                        ...
                                        <td className={style.inputbox}>
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    {clientsAddress.map((item, index) => (
                                                        <li class="dropdown-item " onClick={() => SETNameClientPlaceOnAddress(item)}>{item.ship_Address}</li>
                                                    ))}
                                                </div>
                                            </div>
                                    ...
                                    <tr>
                                        ...
                                        <td className={`${style.inputbox} ${style.pincode}`}>
                                            ...
                                            <select onChange={handleChange} name="dispatch_from_State" value={invoice_fields.dispatch_from_State} className={`col-12 col/sm/7 col/md/7 col/lg/7 col/xl/`} disabled>
                                                <option val="">---State---</option>
                                                {State_list.map((data) => {
                                                    return (
                                                        <option value={`${data.statecode}-${data.statenm}`}>{data.statenm}</option>
                                    ...
                                    <tr>
                                        ...
                                        <td className={style.inputbox}>
                                            ...
                                            <div class="dropdown">
                                                ...
                                                <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                                    {custumersAddress.map((item, index) => (
                                                        <li class="dropdown-item " onClick={() => SETNameCustomerPlaceOnAddress(item)}>{item.ship_Address}</li>
                ...
                <div className={`${style.row}`}>
                    ...
                    <div className={style.productdetails}>
                        <ProductDetails
                            ...
                            setResultAfterPopUp={ResultAfterPopUp}
                        ...
                        <div className={`${style.docdetails}`}>
                            <div className={`${style.row}`}>
                                ...
                                <div className='col-12 col-sm-12 col-md-12 col-lg-6 col-xl-6 d-flex'>
                                    ...
                                    <span className="w-50">
                                        <label className={`${style.translabel}`}>Approx. Distance in km</label>
                ...
                <div className={`${style.row} ${style.buttons}`}>
                    {/* <InvoicePrintTemplate onClick={() => window.print()} invoiceData={invoice_fields}> */}
                    {/* <button type="button" className="btn btn-warning" onClick={() => window.print()}>Preview</button> */}
                    {/* <button type="button" className="btn btn-warning" onClick={() => console.log(InvoiceDATAToPrint)}>Console</button> */}

                    {/* {(option === "View" && invoice_fields.amended) && <button type="button" className="btn btn-warning" onClick={() => window.print()}>Preview</button>} */}
                    {ButtonStatus ? <>
                        {(option !== "View" && !invoice_fields.amended) && <button type="button" className={`btn btn-secondary w-10`} >Save & Preview</button>}
                        {(option !== "View" && !invoice_fields.amended) && <button type="button" className="btn btn-secondary" >Submit</button>}
                    </> : <>
                        {(option !== "View" && !invoice_fields.amended) && <button type="button" className={`btn btn-warning w-10`} onClick={() => ClickedButton("SaveAndPreview")}>Save & Preview</button>}
                        {(option !== "View" && !invoice_fields.amended) && <button type="button" className="btn btn-info" onClick={() => ClickedButton("Submit")}>Submit</button>}
                    </>}
...

            {
                dividedData.map((item, index) => (
                    <InvoicePrintTemplate invoiceValues={InvoiceDATAToPrint} InvoiceItemData={item} />
                ))
            }
