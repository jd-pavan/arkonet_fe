...
function ProductDetails(props) {
  ...
  const [functionAfterPopUp, setFunctionAfterPopUp] = useState(false);
  // console.log(FinalProductData)
  props.sendDataToParent(FinalProductData)
  // props.setResultAfterPopUp(functionAfterPopUp)
  ...
  async function createProductItems(id, invoiceid) {
    ...
    if (!response.ok) {
      ...
      if (props.ClickedButton === "Submit") {
        await Swal.fire("Record save Successfully")


        setFunctionAfterPopUp(true)
    ...
    try {
      ...
      if (failedRecords1.length > 0) {
        ...
        if (props.ClickedButton === "Submit") {
          ...
          Swal.fire({ title: `Update successful.` }).then(async () => {
            console.log(finalProductList)
            props.sendDataToParent(finalProductList);
            setFunctionAfterPopUp(true)
  ...
  function deletProduct(itemid) {
    ...
    // console.log(productlist.filter((item) => item.id !== itemid))
    AssignData(productlist.filter((item) => item.id !== itemid));
  ...
  function addProduct(e) {
    ...
    if (props.option === "View") {
      ...
      const addid = (props.option === "Create" || productlist.length <= 0) ?
  ...
  async function fetchProductDetails(totals) {
    ...
    try {
      const response = await fetch(`${url_}/list/product/ofinvoice/findByBillFromGSTIN?bill_from_GSTIN=${myGSTN}&Invoicemonth=${month}&Invoiceyear=${year}&invoiceid=${invoiceid}`, requestOptions)
  ...
  async function fetchAmendedProductDetails(totals) {
    ...
    try {
      const response = await fetch(`${url_}/list/amend/product/ofinvoice/findByBillFromGSTIN?bill_from_GSTIN=${myGSTN}&Invoicemonth=${month}&Invoiceyear=${year}&invoiceid=${invoiceid}`, requestOptions)
      if (response.status === 200) {
        const result = await response.json();
        console.log(result)
  ...
  function changeProductDetails(option, totals) {
    if (option === "Original") {
      ...
      AssignData(originalproductlist)

    }
    else if (option === "Amended") {
      setProductList(amendedproductlist)
      AssignData(amendedproductlist)
  ...
  return (
    <>
      <table className={`table  ${style.itemtable} ${style.products}`}>
        ...
        <tbody>
          <tr>
            ...
            <th className={style.addelicon}>
              <label><i className="fa-solid fa-square-plus" style={{ "color": "#5550A2" }} onClick={addProduct}></i></label>
            </th>
          </tr>
