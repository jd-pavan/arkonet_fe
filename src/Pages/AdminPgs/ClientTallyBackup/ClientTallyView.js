  ...
  const downloadClientBackup = async (fileid) => {
    ...
    if (subscription_status === "grace_period") {
      ...
      try {
        ...
        try {
          const response = await fetch(`${url_}/getclientbackupfile/${client_pan}`, requestOptions);
          const result = await response.blob()
          if (response.status === 200) {
            if (index !== -1) { saveAs(result, TallyFile[index].filename); }
        ...
        <div className={`d-flex`} >
          <Link className={`${style.ancher}`}
            onClick={(e) => {
              ...
              <i className="fa-solid fa-angle-left"></i>&nbsp;&nbsp;</span></Link>
          ...
          <div className={`d-flex ${style.uploadcontainer}`}>
            ...
            {(uploder === "user" && item.id === "user_tally_file") &&
              <div className={`${style.mainport}`} >
                ...
                <p className='text-gray-400 '>Files Should be less than {maxSize} MB</p>
