...
import ClientFileBackup from "../../ClientMobilePgs/ClientFileBackup/ClientFileBackup";
...
function GstFolder() {
  ...
  const [lastFiveYearsArray, setLastFiveYearsArray] = useState([])
  ...
  const FYyearData = async () => {
    try {
      ...
      const response = await fetch(`${url_}/findFiledNotFiled/For/YearData?userId=${user_id}&clientId=${client_Information.clientId}`, requestOptions)
      const result = await response.json();
      const distinctFinancialYears = [...new Set(result.map(item => item.financialYear))];
      // console.log(distinctFinancialYears)
      setLastFiveYearsArray(distinctFinancialYears)
    } catch (error) {
      console.log(error)
    }
  }
          ...
          {lastFiveYearsArray.map((year, index) => {
            return (
              ...
              <div className='col-6 B' onClick={(e) => {
                e.preventDefault();
                Navigate("gstmonthly", { state: { year: `${Number(year)}-${(Number(year) + 1).toString().slice(-2)}`, client_Information: client_Information } });
              }} key={index}>
                <div className={`${style.uniclass} ${style[`card${index + 1}`]}`}>
                  ...
                  <div className={`${style.textual} `}>
                    <div className={`${style.uptext} `}>
                      <h5 style={{ color: `${getFolderColor(index)}` }}>FY {`${Number(year)}-${(Number(year) + 1).toString().slice(-2)}`}</h5>
                    </div>
                  </div>
                </div>
              </div>
