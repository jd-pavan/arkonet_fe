...
function Cards() {
  ...
  return (
    <div className={style.cards}>
      ...
      <div className={`modal fade animate ${style.modal}`} id='informationModal' data-easein="slideUpIn" tabindex="-1" role="dialog" aria-labelledby="exampleModalCenterTitle" aria-hidden="true">
        <div className="modal-dialog" role="document">
          <div className={`modal-content ${style.animate_bottom}`}>
            <div className={`modal-header ${style.modal_header}`}>
              <h5 className={`modal-title ${style.modal_title}`} id="exampleModalLabel">{information.title}</h5>
              <button type="button" className="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
              </button>
            </div>
            <div className={`modal-body ${style.modal_body}`}>
              <h2 className={`${style.modal_h2}`}>{information.header}</h2>
