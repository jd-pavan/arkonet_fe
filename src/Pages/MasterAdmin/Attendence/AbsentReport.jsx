...
const AbsentReport = () => {
  ...
  const GetCheckInList = async () => {
    try {
      ...
      const response = await fetch(`${url_}/working-time-SubUser-CheckOutList-DateBetween?startDate=${AbsentDate}&endDate=${AbsentDate}&category=User&userid=`, requestOptions)
  ...
  const GetData = async (checkInData) => {
    try {
      ...
      const resultArray = SubUserResult.filter(itemA => !checkInData.some(itemB => itemB.name === itemA.registration.name || itemA.registration.name === null));
